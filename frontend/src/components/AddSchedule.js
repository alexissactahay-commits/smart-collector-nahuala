...
const AddSchedule = () => {
  ...
  const fetchRouteDates = async () => {
    try {
      ...
      if (err.response?.status === 401) handle401();
      else prettyBackendError(err, "Error al cargar fechas");
    }
  };
  ...
  const availableDaysForSelectedRoute = useMemo(() => {
    ...
    unique.sort(
      (a, b) => DAY_ES.indexOf(a) - DAY_ES.indexOf(b)
    );
  ...
  const handleSubmit = async (e) => {
    ...
    if (availableDaysForSelectedRoute.length === 0) {
      alert(
        "Esta ruta no tiene fechas asignadas en AddDate."
      );
      return;
    }
    ...
    try {
      ...
      if (err.response?.status === 401) handle401();
      else prettyBackendError(err, "Error al agregar horario");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `${API_URL}/admin/route-schedules/${id}/`,
        authHeaders()
      );
      ...
      if (err.response?.status === 401) handle401();
      else prettyBackendError(err, "No se pudo eliminar");
    }
  };
  ...
  const schedulesToShow = routeId
    ? routeSchedules.filter(
        (rs) =>
          String(
            rs.route?.id ?? rs.route_id ?? rs.route
          ) === String(routeId)
      )
    : routeSchedules;
  ...
  return (
    <div className="add-schedule-container">
      ...
      {/* BOTÓN REGRESAR */}
      <button
        id="add-schedule-small-back-button"
        type="button"
      ...
      <p style={{ marginTop: -6, color: "#555" }}>
        📌 En este módulo solo defines el <b>horario</b>.
        Los <b>días disponibles</b> salen de las fechas
        asignadas en <b>AddDate</b>.
      </p>
      ...
      <form onSubmit={handleSubmit} className="form">
            ...
            >
              ...
              <b>
                {availableDaysForSelectedRoute.length
                  ? availableDaysForSelectedRoute.join(", ")
                  : "Ninguno"}
              </b>
            </div>
          )}
        ...
        <div className="form-group">
          ...
          <select
            ...
            disabled={
              !routeId ||
              availableDaysForSelectedRoute.length === 0
            }
          >
            <option value="">
              {routeId
          ...
          <tbody>
            {schedulesToShow.map((rs) => (
              <tr key={rs.id}>
                ...
                <td>
                  {getScheduleDayLabel(rs)}
                </td>

                <td>
                  {String(rs.start_time || "").slice(0, 5)}
                </td>

                <td>
                  {String(rs.end_time || "").slice(0, 5)}
                </td>
